"""
JavaScript scripts for crawlergo
All JavaScript code is preserved as multi-line string constants.
"""
...
def make_deliver_result(name: str, seq: int, result: str) -> str:
    """生成DeliverResultJS调用代码"""
    return DeliverResultJS % (name, seq, result)


def make_new_frame(name: str, frame_id: str) -> str:
    """生成创建隐藏iframe的JS代码"""
    return NewFrameTemplate % (name, frame_id)


def make_trigger_inline_event(sleep_ms: float) -> str:
    """生成触发内联事件的JS代码"""
    return TriggerInlineEventJS % sleep_ms


def make_trigger_dom2_event(sleep_ms: float) -> str:
    """生成触发DOM2事件的JS代码"""
    return TriggerDom2EventJS % sleep_ms


def make_trigger_javascript_protocol(sleep_ms: float) -> str:
    """生成触发javascript:伪协议的JS代码"""
    return TriggerJavascriptProtocol % (sleep_ms, sleep_ms)


def make_form_node_click(selector: str) -> str:
    """生成表单节点点击的JS代码"""
    return FormNodeClickJS % selector
...
def get_all_scripts():
    ...
    return {
        "TabInitJS": TabInitJS,
        "DeliverResultJS": DeliverResultJS,
        "ObserverJS": ObserverJS,
        "RemoveDOMListenerJS": RemoveDOMListenerJS,
        "NewFrameTemplate": NewFrameTemplate,
        "TriggerInlineEventJS": TriggerInlineEventJS,
        "TriggerDom2EventJS": TriggerDom2EventJS,
        "TriggerJavascriptProtocol": TriggerJavascriptProtocol,
        "FormNodeClickJS": FormNodeClickJS,
    }
